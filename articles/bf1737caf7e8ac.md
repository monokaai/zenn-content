...
- 社内の別プロジェクトでも DWH を構築しており、その構成に寄せて元 DB→S3→DWH という構成にして欲しい
  - （おそらく）別プロジェクトからは S3 上のデータを閲覧してもらう前提があった
...
アプリインフラが AWS 上に構築されていたので、RedShift や Athena の検討もしましたが、後述の「その他の案」で記載している理由により最終的に BigQuery を選定しました。
